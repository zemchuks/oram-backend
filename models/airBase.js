...
Schema.statics.getByCountry = async function (search) {
    // return await this.find({ country: search, isDeleted: false }).sort({ name: 1 });
    return await this.find({ countryId: search, isDeleted: false }).sort({ name: 1 });
}
...
Schema.statics.updateAirBase = async function (data, id) {
...
module.exports = mongoose.model("AirBase", Schema)